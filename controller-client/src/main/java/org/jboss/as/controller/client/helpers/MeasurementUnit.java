...
/**
 * Metric data values can be in one of the following known units of measurement. These enum values should correspond to the
 * "units" attribute enumerated type values as defined in the plugin descriptor's &ltmetric> element.
 *
...
public enum MeasurementUnit {
    ...
    MeasurementUnit(String displayUnits, Family family, Scale scale) {
    ...
    public static Double calculateOffset(MeasurementUnit first, MeasurementUnit second)
        ...
        if (first.isComparableTo(second) == false) {
            throw new MeasurementConversionException(
                    ControllerClientLogger.ROOT_LOGGER.cannotConvert(first.name(), second.name()));
        }
    ...
    private enum Scale {
        ...
        Scale(Type type, Scale comparisonScale, double offset) {
            this(type);
        ...
        static Double calculateOffset(Scale first, Scale second) {
            if (first.type != second.type) {
                /*
                 * special return value meaning invalid comparison, though it was designed that this method would only be
                 * calling by the outer enumeration, which wouldn't do the necessary checks in advance. so this code path will
                 * rarely, if ever, be followed.
                 */
                return null;
            }
